...
export default function StrukturOrganisasiDinamis() {
  ...
  return (
    <main className="min-h-screen bg-[#FDFBF3] pb-24 overflow-hidden relative"> 
      ...
      {/* 2. ELEMEN DEKORATIF WANITA BERHIJAB & AKADEMIK (LEBIH BANYAK) */}
      ...
      <div className="absolute bottom-[500px] right-[2%] text-[130px] opacity-[0.03] pointer-events-none hidden lg:block">🧕</div>

      {/* 3. BLUR GLOWS (WARNA WARNI LEMBUT) */}
      ...
      <div className="absolute bottom-0 left-0 w-[400px] h-[400px] bg-orange-100/20 rounded-full blur-[100px] pointer-events-none" />
          ...
          <>
            ...
            <div className="flex justify-center mb-32">
              {yayasanList.length > 0 ? yayasanList.map((y, i) => (
                <div key={i} className="bg-white p-8 rounded-[3.5rem] shadow-[0_40px_80px_-15px_rgba(0,0,0,0.1)] border-t-8 border-yellow-500 w-full max-w-sm text-center hover:scale-105 transition-all duration-500 group relative">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12 mb-40 max-w-6xl mx-auto px-4">
              {pimpinanSekolah.map((staff, i) => (
                <div key={i} className="bg-white/80 backdrop-blur-md p-8 rounded-[3rem] shadow-2xl border-l-[12px] border-green-900 flex flex-col md:flex-row items-center gap-8 hover:shadow-green-900/10 hover:-translate-y-2 transition-all group">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-8 md:gap-12">
                {staffLainnya.map((staff, i) => (
                  <div key={i} className="bg-white/70 backdrop-blur-sm p-6 rounded-[3rem] border border-white shadow-[0_20px_50px_rgba(0,0,0,0.05)] hover:shadow-2xl hover:-translate-y-4 transition-all text-center group">
      ...
      {/* FOOTER SECTION RAMAI */}
      <div className="max-w-5xl mx-auto mt-56 text-center px-6 relative pb-20">
        <div className="absolute top-0 left-0 text-6xl opacity-10 rotate-12">🍃</div>
        <div className="absolute bottom-0 right-0 text-6xl opacity-10 -rotate-12">🍃</div>
